...
questions.js
-----
+ List of questions which is stored as an array. Each question contains a number, question, 4 multiple choice options, and the correct answer
```js
...
quizApp.js
-----
+ Declare const variables to get all required info from the buttons /user selections in html
```js
...
+ Functions to the control the buttons within the app
```js
...
+ Functions to pull the questions from the array and display them to the user. Provides the ability for user to interact the the questions and select an answer. 
```js
...
+ Function to display the result box which takes in the results from the questions that were answered in the quiz
```js
...
+ Function to control the 15 second timer and related elements
```js
...
+ Function to show the progress bar that matches the timer
```js
...
+ Creates a span tag that contains the question count
```js
